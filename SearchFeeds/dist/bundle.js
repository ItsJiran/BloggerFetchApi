...
class BlogPostEntity {
  ...
  */
  ...
  buildAuthors(authors) {
    let cont = [];
    for (let author of authors) cont.push(new BlogAuthorEntity(author));
    return cont;
  }
...
class BlogAuthorEntity {
  ...
  */
  constructor(author_entry) {
    ...
    this.embeds = ['email', 'image', 'name', 'uri'];
  }
}
module.exports = {
  BlogPostEntity,
...
class BlogSearchInfo {
  ...
  buildLabel(path = window.location.pathname) {
    ...
    if (paths[1] == 'search' && paths[2] == 'label') this.label = paths[3];
  }
}
class BlogSearchApi {
  ...
  build(blog_info, api_queries = '?') {
    if (blog_info instanceof BlogSearchInfo == false) throw Error('Blog info should be instance of BlogSearchInfo');
...
class Queries {
  ...
  buildByUrl(path = window.location.search) {
    ...
    for (let key in this.queries) {
      if (params.get(key) != undefined) this.setQueries(key, params.get(key));
    }
  }
  ...
  fillApi(path = '?') {
    for (let key in this.queries) {
      ...
      if (!this.isFillApi(key) || !this.fillValidation(key, this.queries[key])) continue;

      // if converted 
      if (this.isConverted(key) != undefined) fill_key = this.isConverted(key);

      // prevent weird url
      if (path.length != 1) path += '&';

      // if not array
      if (!Array.isArray(this.queries[key])) path += fill_key + '=' + this.middleware(key, this.queries[key]);
  ...
  isFillApi(key) {
    if (this.rules[key] != undefined) {
      if (!this.rules[key].fillApi) return false;
    }
    return true;
  }
  fill(path = '?') {
    for (let key in this.queries) {
      ...
      // skip if shouldn't filled
      if (!this.isFill(key) || !this.fillValidation(key, this.queries[key])) continue;
  ...
  isFill(key) {
    if (this.rules[key] != undefined) {
      if (!this.rules[key].fill) return false;
    }
    return true;
  }
  isConverted(key) {
    if (this.rules[key] != undefined) {
      if (this.rules[key].converted != undefined) return this.rules[key].converted;
    }
    return undefined;
  }
  ...
  setQueries(key, value) {
    if (this.validation(key, value)) this.queries[key] = value;
  }
...
class BlogPagination extends Queries {
  ...
  // middleware
  middleware(key, value) {
    if (key == 'start_index') value = parseInt(this.queries['start_index']) + (parseInt(this.queries['current_page']) - 1) * parseInt(this.queries['max_results']);
    return value;
  }
...
(() => {
"use strict";
/* harmony import */ var _builder__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(990);
/* harmony import */ var _builder__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_builder__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _entity__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(874);
/* harmony import */ var _entity__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_entity__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var _info__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(765);
/* harmony import */ var _info__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_info__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _queries__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(673);
/* harmony import */ var _queries__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_queries__WEBPACK_IMPORTED_MODULE_3__);
...
class BlogSearch {
  constructor(object = {}) {
    this.BlogSearchInfo = new _info__WEBPACK_IMPORTED_MODULE_2__.BlogSearchInfo(object.location);
    this.BlogSearchApi = new _info__WEBPACK_IMPORTED_MODULE_2__.BlogSearchApi(this.BlogSearchInfo);
    this.BlogSearchQueries = new _queries__WEBPACK_IMPORTED_MODULE_3__.BlogSearchQueries(object.search_settings);

    // Entity or Filtered Element
    this.BlogPosts = [];
    this.BlogPagination = new _queries__WEBPACK_IMPORTED_MODULE_3__.BlogPagination(object.pagination_settings);
  ...
  buildPostEntity(response) {
    ...
    for (let post of response.feed.entry) {
      this.BlogPosts.push(new _entity__WEBPACK_IMPORTED_MODULE_1__.BlogPostEntity(post));
    }
  }
  ...
  buildPaginationQueries(path = '?', page = '#') {
    ...
    if (path == '?') return path + 'current_page=' + page;else return path + '&current_page=' + page;
  }
  printPostsEntity() {
    if (this.BlogPosts.length <= 0) {
      let element = _builder__WEBPACK_IMPORTED_MODULE_0__.EmptyPosts.build();
      this.posts_container.appendChild(element);
    } else {
      for (let post of this.BlogPosts) {
        let element = _builder__WEBPACK_IMPORTED_MODULE_0__.PostBuilder.build(post);
        // print aditional element
        if (post.labels >= 1) {
          let label_element = _builder__WEBPACK_IMPORTED_MODULE_0__.PostLabelBuilder.build(post.labels[0]);
  ...
  buildPaginationElement(title, link) {
    let entity = new _entity__WEBPACK_IMPORTED_MODULE_1__.BlogPaginationEntity({
      title: title,
      link: link
    });
    return _builder__WEBPACK_IMPORTED_MODULE_0__.PaginationLink.build(entity);
  }
  resetEntity() {
    this.BlogPosts = [];
  }
